...
/// use nextshell::Filter;
///
/// let hello = nextshell::path("hello")
///     .map(nextshell::reply)
///     .with(nextshell::trace::named("hello"));
///
/// let goodbye = nextshell::path("goodbye")
///     .map(nextshell::reply)
///     .with(nextshell::trace::named("goodbye"));
///
...
mod internal {
    ...
    fn finished_logger<E: IsReject>(reply: &Result<(Traced,), E>) {
        ...
        if status.is_success() {
            tracing::info!(
                target: "nextshell::filters::trace",
        ...
        F::Error: IsReject,
    {
        ...
        fn filter(&self, _: Internal) -> Self::Future {
            ...
            tracing::info!(target: "nextshell::filters::trace", "processing request");
            self.filter
                .filter(Internal)
