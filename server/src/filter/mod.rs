...
pub trait Filter: FilterBase {
    ...
    ///
    /// // Match `/hello/:name`...
    /// nextshell::path("hello")
    ///     .and(nextshell::path::param::<String>());
    /// ```
    ...
    /// nextshell::path::param::<u32>()
    ///     .or(nextshell::path::param::<SocketAddr>());
    /// ```
    ...
    ///
    /// // Map `/:id`
    /// nextshell::path::param().map(|id: u64| {
    ///   format!("Hello #{}", id)
    /// });
    /// ```
    ///
    ...
    ///
    /// // Map `/:id`
    /// nextshell::path::param().then(|id: u64| async move {
    ///   format!("Hello #{}", id)
    /// });
    /// ```
    ...
    ///
    /// // Validate after `/:id`
    /// nextshell::path::param().and_then(|id: u64| async move {
    ///     if id != 0 {
    ///         Ok(format!("Hello #{}", id))
    ///     } else {
    ///         Err(nextshell::reject::not_found())
    ///     }
    /// });
    /// ```
    ...
    /// use nextshell::Filter;
    ///
    /// let client_ip = nextshell::header("x-real-ip")
    ///     .or(nextshell::header("x-forwarded-for"))
    ///     .unify()
    ///     .map(|ip: SocketAddr| {
    ...
    /// but just `()`, since nextshell will wrap it up into a `((),)`.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// let route = route.with(nextshell::log("example"));
    /// ```
    ...
    /// use nextshell::Filter;
    ///
    /// fn impl_reply() -> nextshell::filters::BoxedFilter<(impl nextshell::Reply,)> {
    ///     nextshell::any()
    ///         .map(nextshell::reply)
    ///         .boxed()
    /// }
    ///
    /// fn named_i32() -> nextshell::filters::BoxedFilter<(i32,)> {
    ///     nextshell::path::param::<i32>()
    ///         .boxed()
    /// }
    ///
    /// fn named_and() -> nextshell::filters::BoxedFilter<(i32, String)> {
    ///     nextshell::path::param::<i32>()
    ///         .and(nextshell::header::<String>("host"))
    ///         .boxed()
    /// }
    /// ```
