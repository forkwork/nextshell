...
//!
//! ```
//! use nextshell::{reply, Reply, Filter, reject, Rejection, http::StatusCode};
//!
//! #[derive(Debug)]
...
//!         if id == 0 {
//!             Err(nextshell::reject::custom(InvalidParameter))
//!         } else {
//!             Ok("id is valid")
//!         }
//!     })
...
///
/// ```compile_fail
/// fn with(r: nextshell::Rejection) {
///     let _wat = nextshell::reject::custom(r);
/// }
/// ```
...
///
/// ```
/// use nextshell::{Filter, reject::Reject};
///
/// #[derive(Debug)]
/// struct RateLimited;
///
...
///
/// let route = nextshell::any().and_then(|| async {
///     Err::<(), _>(nextshell::reject::custom(RateLimited))
/// });
/// ```
...
impl Rejection {
    ...
    /// struct Nope;
    ///
    /// impl nextshell::reject::Reject for Nope {}
    ///
    /// let reject = nextshell::reject::custom(Nope);
    ///
    ...
    /// Returns true if this Rejection was made via `nextshell::reject::not_found`.
    ///
    /// # Example
    ///
    /// ```
    /// let rejection = nextshell::reject();
    ///
...
mod sealed {
    ...
    pub trait CombineRejection<E>: Send + Sized {
        ...
        /// # For example:
        ///
        /// `nextshell::any().and(nextshell::path("foo"))` has the following steps:
        ///
        /// 1. Since this is `and`, only **one** of the rejections will occur,
        ///    and as soon as it does, it will be returned.
        /// 2. `nextshell::any()` rejects with `Never`. So, it will never return `Never`.
        /// 3. `nextshell::path()` rejects with `Rejection`. It may return `Rejection`.
        ///
